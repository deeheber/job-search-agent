...
**Tools**: Community tools from `strands_tools` package (`http_request`, `current_time`)
...
**Tool Imports**: `from strands_tools import current_time, http_request`
...
Default model is defined in `agentcore_app.py` as `DEFAULT_MODEL_ID`. Override with environment variable:
